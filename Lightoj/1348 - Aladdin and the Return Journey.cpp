...
#push_back define pii pair<int,int> define md 
#1000000007 define mm(x,y) 
#memset(x,y,sizeof(x)) define fs first define 
#sc second define N 100005
 
 
using namespace std;
 
int 
n,par[N],lev[N],chainhead[N],chainind[N],serial[N],ara[N],val[N],tree[4*N],subsize[N],chain,pos;
...
void update(int node,int s,int f,int in,int 
val) {
    if(s>in || f<in) return;
    if(s==f)
    {
...
int query_up(int u,int v) {
    int 
ans=0,uchain=chainind[u],vchain=chainind[v];
...
scanf("%d",&val[i]);
        ...
        chain=1;
        hld(1,-1);
        build(1,1,n); // for(int i=1;i<5;i++) 
printf("%d ",ara[i]); // cout<<endl; // 
...
4 10 20 30 40 0 1 1 2 1 3 3 0 2 3 1 1 100 0 2 
3
